/*
Задача 20.
Направил съм QUEUE вариацията, та ще се запипа и на втората юаст от С++ курса за OOP :)

Напишете програма, която да създаде стек (FIFO) от N елемента,
...
int pop(int *StackPointer, int *Position){
    ...
    if (Position >= 0){
        for(int i = 0; i<*Position; i++){
            StackPointer[i] = StackPointer[i+1];
        }
        StackCounter(Position, PushDirection);
